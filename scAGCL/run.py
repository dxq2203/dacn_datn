...
from model import Encoder, AGCLModel, NewModel, NewModel2
from utils_run import load_h5_data1, load_h5_data2, GraphConstruction
from train_run import Train
from sklearn.cluster import KMeans
from sklearn.metrics import (adjusted_rand_score,  normalized_mutual_info_score)
from sklearn.preprocessing import normalize
from sklearn.decomposition import PCA 
from GATEncoder import GATEncoder, GATv2Encoder, GATEncoderDense
...
if __name__ == '__main__':
    ...
    #Build a model
    encoder = GATEncoderDense(data.num_features, num_hidden, out_features=256, num_heads=4, dropout_prob=0.3).to(device)
    model = NewModel2(encoder, num_hidden, num_proj_hidden, tau).to(device)
    ...
    # ## PCA-init
    # print('Initialization using PCA for PGD')
    # n_components = 40 
    # pca = PCA(n_components=n_components)
    # pca.fit(X)
    # principal_components = torch.from_numpy(pca.components_).float().to(device)

    # import pdb; pdb.set_trace()
    embeddings=Train(model, data, cellGraph, device, num_epochs,lam, num_cluster,alpha, beta, num_itersAdv, optimizer, edge_r1, edge_r2, feature_r1, feature_r2, subgraph_size, principal_components=None)
